...
class AppRepositoryImpl(
    ...
    private fun routeOptions(coords: List<DoubleArray>): RouteOptions {
        ...
        builder.accessToken(local.getAccessToken())
            .baseUrl("")
            .user("test")
            .bearings(";")
            .coordinates(
...
//        RouteOptions.fromJson(TEMP)
        ...
        try {
            route.steps?.apply {
                ...
                for (i in 0 until size - 1) {
                    ...
                    if (i == 0) {
                        subPath.add(Point.fromLngLat(p1[1] + 0.0009, p1[0] + 0.0009))
                    }
    ...
    private fun generateFinalStep(point: Point): LegStep {
        ...
        val stepBuilder = LegStep.builder()
            .distance(0.0)
            .duration(0.0)
            .geometry(PolylineUtils.encode(listOf(point), PRECISION_6))
            .name("Đến nơi")
            .drivingSide("right")
            .mode("driving")
            .intersections(listOf(intersection))
    ...
    private fun getGuide(turn: Int, isBanner: Boolean): Pair<String, String> {
        ...
        if (turn == 11) {
            return if (!isBanner)
                (StepManeuver.ROTARY to STEP_MANEUVER_MODIFIER_RIGHT)
            else
                (StepManeuver.ROUNDABOUT to STEP_MANEUVER_MODIFIER_RIGHT)
        }

        if (turn == 12 && isBanner) {
            return (StepManeuver.TURN to STEP_MANEUVER_MODIFIER_RIGHT)
        }
        ...
        return when (turn) {
            ...
            12 -> (StepManeuver.EXIT_ROTARY to STEP_MANEUVER_MODIFIER_RIGHT)
